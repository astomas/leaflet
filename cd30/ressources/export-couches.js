	...
	// [cd30] Détermine si une couche métier (tabCoucheMG) est active : l'œil vert
	// de la sidebar équivaut à la couche présente sur la carte (map.hasLayer).
	// Le repli sur le registre modernLegendeParCouche (version sidebar-légende
	// des cartes racine) marquait à tort TOUTES les couches comme affichées ici,
	// car les groupes de légende sont sur la carte dès l'ouverture en cd30.
	function coucheMetierAffichee(map, couche) {
		return map.hasLayer(couche);
	}
