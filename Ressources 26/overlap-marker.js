...
modif cd30 tailles des liens, modif liens si icones  <> punaises , pas de unspiderfy si clic controles leaflet (case a cocher)
    */
    (function() {
			...
			d.spiderfy = function(c, b) {
                ...
                a = function() {
                    ...
                    for (b = d.length; a < b; a++) {
                        ...
                        // --- DEBUT MODIFICATION ---
                        var startLatLng = h.getLatLng();
                        var endLatLng = e;

                        // Vérification de l'icone et du dossier pour centrer ou non
                        if (h.options.icon && h.options.icon.options) {
                            var iconUrl = h.options.icon.options.iconUrl || "";
                            var cheminDossierPunaises = "/Ressources/Images/Punaises";
                            
                            // Si l'image n'est PAS dans le dossier 'Punaises', on calcule le centre
                            if (iconUrl.indexOf(cheminDossierPunaises) === -1) {
                                if (h.options.icon.options.iconAnchor && h.options.icon.options.iconSize) {
                                    // Calcul du décalage pour remonter de la base vers le centre
                                    // Offset = AncreY - (Hauteur / 2)
                                    var offset = h.options.icon.options.iconAnchor[1] - (h.options.icon.options.iconSize[1] / 2);
                                    
                                    // Application du décalage aux points de départ et d'arrivée (en pixels)
                                    var startPt = m.map.latLngToLayerPoint(startLatLng);
                                    startPt.y -= offset;
                                    startLatLng = m.map.layerPointToLatLng(startPt);

                                    // Pour la destination 'g', on peut modifier directement ou convertir
                                    // Ici on utilise 'g' qui est le point pixel d'arrivée original
                                    var endPt = new L.Point(g.x, g.y);
                                    endPt.y -= offset;
                                    endLatLng = m.map.layerPointToLatLng(endPt);
                                }
                            }
                        }

                        p = new L.Polyline([startLatLng, endLatLng], {
                            color: this.legColors.usual,
                            weight: this.legWeight,
                            clickable: !1 // Garde la ligne non-cliquable (évite le curseur main)
                        });
                        // --- FIN MODIFICATION ---
                        ...
                        this.legColors.highlighted !== this.legColors.usual && (l = this.makeHighlightListeners(h), h._omsData.highlightListeners = l, h.addEventListener("mouseover", l.highlight), h.addEventListener("mouseout", l.unhighlight));
                        h.setLatLng(e);
